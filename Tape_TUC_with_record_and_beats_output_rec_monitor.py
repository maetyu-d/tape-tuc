...
class RecordIODevice(QIODevice):
    def __init__(self, looper):
        super().__init__()
        self.looper = looper
    ...
    def writeData(self, data: bytes) -> int:
        if data:
            self.looper.append_record_data(data)
        return len(data)
...
class TapeLooper(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        top.addWidget(self.btn_stop_rec)
        ...
        self.beat_sens_lbl = QtWidgets.QLabel("Beat Sens 50")
    ...
    def start_recording(self):
        ...
        with self.lock:
            ...
            self.record_sample_rate = self.sample_rate
            self.is_recording = True

        # Auto-start playback for monitoring if audio is loaded
    ...
    def stop_recording(self):
        ...
        if self.record_device is not None and self.record_device.isOpen():
            self.record_device.close()
        self.audio_input = None
        self.record_device = None
        self.is_recording = False

        if not self.record_buffer or self.record_sample_rate <= 0 or not self.record_path:
            return

        data_int16 = np.frombuffer(bytes(self.record_buffer), dtype=np.int16)
        if data_int16.size == 0:
            return
        data_float = data_int16.astype(np.float32) / 32768.0
        try:
            sf.write(self.record_path, data_float, self.record_sample_rate, subtype="PCM_16")
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Error writing WAV", str(e))
            return

        load = QtWidgets.QMessageBox.question(
            self,
            "Recording finished",
            "Recording saved.\nLoad it into Tape TUC?",
            QtWidgets.QMessageBox.StandardButton.Yes | QtWidgets.QMessageBox.StandardButton.No,
            QtWidgets.QMessageBox.StandardButton.Yes,
        )
        if load == QtWidgets.QMessageBox.StandardButton.Yes:
            try:
                data, sr = sf.read(self.record_path, dtype="float32", always_2d=True)
                if data.shape[1] > 1:
                    mono = data.mean(axis=1)
                else:
                    mono = data[:, 0]
                ...
                QtWidgets.QMessageBox.critical(self, "Error loading recording", str(e))
    ...
    def on_sens_change(self, v):
        self.beat_sens_lbl.setText(f"Beat Sens {v}")
    ...
    def _apply_state(self, state):
        with self.lock:
            ...
            self.anticlick_amount = int(state.get("anticlick_amount", self.anticlick_amount))
        ...
        try:
            ...
            self.anticlick_slider.setValue(self.anticlick_amount)
    ...
    def provide_samples(self, frames: int) -> np.ndarray:
        ...
        with self.lock:
            ...
            if self.is_recording:
                self.record_buffer.extend(out.tobytes())
    ...
    def detect_beats(self):
        ...
        sens = self.beat_sens.value() / 100.0
        thresh = mean + sens * std
